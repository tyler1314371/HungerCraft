...
#using python flask framework to host our backend
app = Flask(__name__, template_folder='E:\\HungerCraft\\angular' , static_url_path='')
cors = CORS(app)
app.config['CORS_HEADERS'] = 'Content-Type'
...
@app.route('/app/js/<path:path>')
def send_js(path):
	return send_from_directory('E:\\HungerCraft\\angular\\app\\js', path)

@app.route('/assets/<path:path>')
def send_assets(path):
	return send_from_directory('E:\\HungerCraft\\angular\\assets', path)
